...
const TABLENAME = 'Department Form';

const getDepartmentFormById = async (req: Request, res: Response, next: NextFunction) => {
  logging.info(NAMESPACE, `GETTING A ${TABLENAME.toUpperCase} BY ID`);
  ...
  try {
    const retrieveQuestions = await Knex.select('Department.*', 'Question.*', 'DepartmentQuestion.*')
    ...
    logging.info(NAMESPACE, `GOT ${TABLENAME.toUpperCase} FOR DEPARTMENT ${departmentId}`, retrieveQuestions);
    if (!retrieveQuestions.length) {
      res.status(404).send(departmentDNEError);
      return;
    }
    res.send(retrieveQuestions);
  } catch (error: any) {
